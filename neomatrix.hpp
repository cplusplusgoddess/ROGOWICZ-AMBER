...
//		Initialization or fill	: initialize_random() Method random init _data of a Matrix distribution range 0.0-9.9
//                 				: Matrix A[4][2]  = 1.4; - Inits all _data elements to 1.4
//      Init Constructor		: Matrix(const std::vector<std::vector<T>> & v): Init matrix _data with a std::vector 
...
// #define NEOMATLIB_ALIGN(a) __attribute__((aligned(a)))
// #define NEOMATLIB_ENSURE_INLINE __attribute__((always_inline))
...
namespace neolib
{
	...
	template <typename T = double> class NeoMatrix
	{
		...
		// Initialize matrix _data with a std::vector 
		...
		// Initialize matrix _data with an array of doubles calling default constructor 1st
...
*/
        ...
        // Compute the number of _rows for each thread to handle computing for the 
		...
		NEOMX multiplyThreaded(const NEOMX & m) const
		{
			...
			{
                // Now hand the work load of block_size (number of _rows to process) to 
                // the threads to handle in the static method multiplyBlock() passing
				// references to the valarray _data and the this ptr
			...
			// take care of the last block and tack on any remaining _rows left over from our thread division
			...
			return NEOMX(_rows, m._cols, std::move(result));
       ...
       // multiply - for single thread case
        ...
        // the number of _rows to process
        ///////////////////////////////////////////////
		NEOMX operator*(const NEOMX & m) const
        {
		...
		///////////////////////////////////////////
        // transpose() swap the _rows and columns
        // of this matrix
